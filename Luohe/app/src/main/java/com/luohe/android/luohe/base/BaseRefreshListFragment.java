 ...
 */
public abstract class BaseRefreshListFragment<T> extends BaseListFragment<T> implements
	...
	public final int MODE_DISABLE = 1;

	public final int MODE_LOADMORE = 0;

	protected void setListMode(int MODE) {
	...
	private EndlessRecyclerOnScrollListener mOnScrollListener = new EndlessRecyclerOnScrollListener() {
		...
		public void onLoadNextPage(View view) {
			...
			if (m_SIZE < TOTAL_COUNTER) {
				// loading more
				RecyclerViewStateUtils.setFooterViewState(getActivity(), getRecyclerView(), m_PAGE_SIZE,
	...
	public void notifyLoadComplete(int errorCode) {
		if (getSwipeRefreshLayout().isRefreshing()) {// 刷新后
			...
			if (errorCode != Result.OK) {
				...
				RecyclerViewStateUtils.setFooterViewState(getActivity(), getRecyclerView(), m_PAGE_SIZE,
						LoadingFooter.State.NetWorkError, mFooterClick);
			}
			// up todo
			// do nothing
		}
	...
	protected abstract void onPullDown(final AdapterManger<T> mAdapterManger);
